select * from (
SELECT  
	ROW_NUMBER() OVER (PARTITION BY historial_envio.codigo_cliente, YEAR(historial_envio.dw_creationdate),month(historial_envio.dw_creationdate)   ORDER BY historial_envio.dw_creationdate )	AS ORD,
...
WHERE
	 info_campania.start_date >= '2026-01-01 00:00:00'
	AND info_campania.start_date <  '2026-04-01 00:00:00'
	AND info_campania.countryid = 2
	AND (info_campania.name LIKE '%creacion de usuario%' OR info_campania.name LIKE '%creación de usuario%' OR info_campania.name LIKE '% cu %')
	AND info_campania.description LIKE '%reg sqvd%'
	AND historial_envio.dw_creationdate >= '2026-01-01 00:00:00'
