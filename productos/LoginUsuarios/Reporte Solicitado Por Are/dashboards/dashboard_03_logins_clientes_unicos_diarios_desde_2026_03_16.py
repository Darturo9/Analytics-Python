"""
dashboard_03_logins_clientes_unicos_diarios_desde_2026_03_16.py
---------------------------------------------------------------
Dashboard 03 (Reporte Solicitado Por Are):
- Grafico diario de clientes unicos con login desde 2026-03-16.
- Sin resaltado por color de campanas.

Ejecucion:
    python3 "productos/LoginUsuarios/Reporte Solicitado Por Are/dashboards/dashboard_03_logins_clientes_unicos_diarios_desde_2026_03_16.py"
"""
...
PORT = 8064
COLOR_BARRA = COLORES["aqua_digital"]
PLACEHOLDER_CLIENTES_VALUES = "{{CLIENTES_VALUES}}"

SQL_LOGINS_UNICOS_DIARIOS = """
WITH clientes AS (
    SELECT DISTINCT
...
SELECT
    ...
    t.clientes_unicos_con_login_periodo
FROM base b
CROSS JOIN (
    SELECT
        COUNT(*) AS total_eventos_login_periodo,
        COUNT(DISTINCT padded_codigo_usuario) AS clientes_unicos_con_login_periodo
    FROM base
) t
GROUP BY
    b.fecha,
...
def cargar_logins_unicos_diarios(
    ...
    if not codigos_validos:
        return pd.DataFrame(columns=["fecha", "clientes_unicos_login_dia"]), 0, 0

    values_sql = ",\n        ".join(f"('{codigo}')" for codigo in codigos_validos)
    if PLACEHOLDER_CLIENTES_VALUES not in SQL_LOGINS_UNICOS_DIARIOS:
        raise ValueError(f"La query no contiene placeholder {PLACEHOLDER_CLIENTES_VALUES}.")
    query_sql = SQL_LOGINS_UNICOS_DIARIOS.replace(PLACEHOLDER_CLIENTES_VALUES, values_sql)
    ...
    total_eventos = int(pd.to_numeric(out["total_eventos_login_periodo"], errors="coerce").fillna(0).max())
    clientes_con_login = int(
        pd.to_numeric(out["clientes_unicos_con_login_periodo"], errors="coerce").fillna(0).max()
    )

    return out[["fecha", "clientes_unicos_login_dia"]], total_eventos, clientes_con_login
...
def construir_base_diaria(
    ...
    if logins_diario.empty:
        clientes_unicos = pd.Series(0, index=fechas)
    else:
        diario_idx = logins_diario.set_index("fecha")
        clientes_unicos = diario_idx["clientes_unicos_login_dia"].reindex(fechas, fill_value=0)

    diario = pd.DataFrame({"fecha": fechas})
    diario["clientes_unicos_login"] = clientes_unicos.values
    return diario
...
def construir_figura(diario: pd.DataFrame) -> go.Figure:
    ...
    max_total = int(diario["clientes_unicos_login"].max()) if not diario.empty else 0
    ...
    fig.add_trace(
        go.Bar(
            x=diario["fecha"],
            y=diario["clientes_unicos_login"],
            marker_color=COLOR_BARRA,
            text=[f"{int(v):,}" if int(v) > 0 else "" for v in diario["clientes_unicos_login"]],
            textposition="outside",
            hovertemplate=(
                "Fecha %{x|%Y-%m-%d}<br>"
                "Clientes unicos con login: %{y:,}<extra></extra>"
            ),
            showlegend=False,
        )
    )
    ...
    fig.update_layout(
        ...
        yaxis=dict(title="Clientes unicos con login", range=[0, max_total + (separacion * 3)]),
        bargap=0.15,
    )
    return fig
...
def construir_dashboard() -> Dash:
    ...
    logins_diario, total_eventos_login, clientes_con_login = cargar_logins_unicos_diarios(
    ...
    app.layout = html.Div(
        ...
        children=[
            html.H2(
                "Dashboard 03: Logins Diarios de Clientes Unicos",
            ...
            html.Div(
                ...
                children=[
                    ...
                    kpi_card("Clientes unicos con login", f"{clientes_con_login:,}", COLORES["aqua_digital"]),
            ...
            html.Div(
                ...
                children=[dcc.Graph(id="graf-logins-clientes-unicos-diarios", figure=fig)],
            ),
        ],
    )
...
def main() -> None:
    ...
    print(f"Dashboard 03 corriendo en http://127.0.0.1:{PORT}")
