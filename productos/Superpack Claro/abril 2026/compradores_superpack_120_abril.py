"""
compradores_superpack_120_abril.py
------------------------------------
Reporte de clientes del universo completo de compradores de Superpack Claro
en abril 2026 que realizaron al menos una compra >= L.120 (no revertida).

Ejecucion:
    python3 "productos/Superpack Claro/abril 2026/compradores_superpack_120_abril.py"
"""
...
QUERY_PATH = Path(__file__).resolve().parent / "queries" / "compradores_superpack_120_abril_2026.sql"


def imprimir_resultados(df) -> None:
    if df.empty:
        print("Sin resultados para abril 2026.")
        return
    ...
    print("\n" + "=" * 60)
    print("  SUPERPACK CLARO - COMPRADORES >= L.120 - ABRIL 2026")
    print("  (universo completo, canales 1 y 7, excluye juridicos)")
    print("=" * 60)
    print(f"  Total compradores efectivos abril:    {total:>10,}")
...
def main() -> None:
    try:
        print("Consultando compradores Superpack >= L.120 en abril 2026...")
