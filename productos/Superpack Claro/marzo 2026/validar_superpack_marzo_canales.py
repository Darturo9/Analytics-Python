"""
validar_superpack_marzo_canales.py
------------------------------------
Validacion de compras de Superpack Claro en marzo 2026 por canal de contacto.
...
Ejecucion:
    python3 "productos/Superpack Claro/marzo 2026/validar_superpack_marzo_canales.py"

Argumentos opcionales:
    --fecha-inicio   YYYY-MM-DD  (default: 2026-03-01)
    --fecha-fin      YYYY-MM-DD  (default: 2026-04-01, exclusiva)
"""
...
QUERY_PATH = BASE_DIR / "queries" / "compras_superpack_marzo_2026.sql"
INPUT_CLIENTES = SUPERPACK_DIR / "exports" / "clientes_contactados_unificados_prioridad_rtm.xlsx"
OUTPUT_JSON = BASE_DIR / "exports" / "validacion_superpack_marzo_canales.json"
...
def imprimir_resumen(detalle: pd.DataFrame, compras: pd.DataFrame) -> None:
    ...
    print("\n===== RESUMEN VALIDACION SUPERPACK MARZO 2026 =====")
    ...
    print(f"Universo total compradores marzo (no solo lista): {total_clientes_compradores_universo:,}")
    print(f"Universo total transacciones marzo: {total_tx_universo:,}")
    print(f"Universo monto total marzo: {monto_total_universo:,.2f}")
    print("----------------------------------------------------")
    ...
    print("\nResumen por canal de compra (universo marzo):")
    if compras.empty:
    ...
    if compradores_lista.empty:
        print("Ningun cliente de la lista compro superpack en marzo.")
    else:
        cruce = (
...
def construir_payload_json(detalle: pd.DataFrame, compras: pd.DataFrame) -> dict:
    ...
    return {
        ...
        "resumen_general": {
            ...
            "universo_compradores_marzo": int(compras["codigo_cliente"].nunique()) if not compras.empty else 0,
            "universo_transacciones_marzo": int(compras["trx_weight"].sum()) if not compras.empty else 0,
            "universo_monto_total_marzo": round(float(compras["weighted_monto"].sum()) if not compras.empty else 0.0, 2),
        },
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Validacion compras Superpack marzo 2026 por canal de contacto."
    )
    parser.add_argument("--fecha-inicio", default="2026-03-01", help="Fecha inicio inclusiva (YYYY-MM-DD).")
    parser.add_argument("--fecha-fin",    default="2026-04-01", help="Fecha fin exclusiva (YYYY-MM-DD). Default: 2026-04-01")
    ...
    try:
        ...
        print(f"Ejecutando query de compras marzo (hasta {args.fecha_fin} exclusiva): {QUERY_PATH}")
        compras = cargar_compras_superpack(QUERY_PATH, args.fecha_inicio, args.fecha_fin)
        print(f"Transacciones de superpack marzo: {len(compras):,}")
