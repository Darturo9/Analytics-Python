				...
				GROUP BY CLCCLI
					...
					DW_BEL_IBUSER ON (DW_BEL_IBCLIE.CLCCLI=DW_BEL_IBUSER.CLCCLI) 
					 LEFT JOIN TR_BEL_IBPERF ON (DW_BEL_IBUSER.PEUSUA=TR_BEL_IBPERF.PECODE)
						...
						LEFT JOIN conteo_usuarios cu ON cu.CLCCLI = DW_BEL_IBCLIE.CLCCLI
				) Datos
			WHERE 
				RN=1 
...
(
	...
	INNER JOIN (
		SELECT
			DW_CIF_CLIENTES.DW_SECTOR_DESCRIPCION,
			LTRIM(RTRIM(DW_CIF_CLIENTES.CLDOC)) AS CIF,
			DW_CIF_CLIENTES.CLTIPE AS Tipo_Cliente,
			DW_CIF_DIRECCIONES.DW_NIVEL_GEO1 AS Zona,
			DW_CIF_DIRECCIONES.DW_NIVEL_GEO2 AS Depto,
			DW_CIF_CLIENTES.dw_usuarios_bel_cnt AS BancaE,
			ROW_NUMBER() OVER (PARTITION BY DW_CIF_DIRECCIONES.CLDOC ORDER BY DW_CIF_DIRECCIONES.dw_fecha DESC) AS RN
		...
		WHERE DW_CIF_DIRECCIONES.CLDICO = 1
		  AND DW_CIF_CLIENTES.CLTIPE = 'J'
		  AND DW_CIF_CLIENTES.dw_usuarios_bel_cnt = 1
	) [Tipo de cliente] ON Transacciones.Codigo_Cliente = [Tipo de cliente].CIF
		AND [Tipo de cliente].RN = 1
