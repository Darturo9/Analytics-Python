...
FROM (
	...
	LEFT JOIN
		dw_cif_direcciones correos						-- JOIN para correo electrónico
		ON RTRIM(LTRIM(depositos.cldoc)) = RTRIM(LTRIM(correos.cldoc))
		AND correos.cldico = 4							-- tipo 4 = correo electrónico
		AND correos.cldire LIKE '%_@_%.__%'				-- formato válido de correo
	LEFT JOIN
	...
	WHERE
		depositos.dw_feha_apertura >= '2026-01-01'
