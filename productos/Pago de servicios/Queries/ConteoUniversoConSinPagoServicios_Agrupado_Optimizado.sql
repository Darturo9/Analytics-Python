/*
Conteo optimizado:
- Universo: clientes naturales activos con banca activa (dw_usuarios_bel_cnt > 0)
- Pagos: categorias agrupadas por reglas de descripcionServicios.txt
- Excluye: Pago TC, Pago TC terceros, Transferencias propias, Transferencias a terceros
*/
...
SELECT
    (SELECT COUNT(*) FROM universo_clientes) AS universo_clientes,
    (SELECT COUNT(*) FROM pagadores_unicos) AS clientes_con_pago,
    (SELECT COUNT(*) FROM universo_clientes) - (SELECT COUNT(*) FROM pagadores_unicos) AS clientes_sin_pago;
