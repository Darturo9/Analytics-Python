...
--2 registros
		...
		ELSE DW_BEL_IBSERV.SEDESC + ' - ' + dw_bel_IBSTTRA_VIEW.SECODE END Operaci�n,
	...
	WHERE 
		...
		dw_bel_IBSTTRA_VIEW.dw_fecha_trx BETWEEN '2025-01-01' AND '2025-01-31'

		--BETWEEN '02/01/2025 00:00:00' AND '02/28/2025 23:59:59' 
		-- 2 registros
		--and dw_bel_IBSTTRA_VIEW.CACODE in ( 'App nueva','AP','APP')
...
FROM (
	...
	FROM (
		...
		WHERE 
			...
			CAST(Fecha_hora_entrada AS DATE) BETWEEN '2025-01-01' AND '2025-01-31'
...
---- TRX JOURNAL
	...
	SELECT 
		...
		DW_BEL_IBSERV_ALIAS.SEDESC + ' - ' + dw_BEL_IBJOUR.SECODE Operaci�n,
	...
	(
		...
		dw_BEL_IBJOUR.dw_fecha_journal BETWEEN '2025-01-01' AND '2025-01-31'
...
--preguntar ac�
	...
	WHERE 
		...
		dw_bel_IBHISPLA.dw_fecha_sistema BETWEEN '2025-01-01' AND '2025-01-31'
...
--preguntar ac�
		...
		DW_DEP_DPMOVM_VIEW.dw_transaccion Operaci�n,
	...
	WHERE 
		...
		DW_DEP_DPMOVM_VIEW.dw_fecha_operacion BETWEEN '2025-01-01' AND '2025-01-31'
...
--preguntar ac�
		CASE 
			...
			WHEN TIPPAG = '1' THEN 'Cr�dito a cuentas propias' 
	...
	WHERE 
		...
		dw_bel_IBHLTP.dw_fecha_aplicado BETWEEN '2025-01-01' AND '2025-01-31'
...
--ARTURO
		...
		DW_MUL_SPMACO.SPNOMC AS Operaci�n,
	...
	WHERE
		...
		DW_MUL_SPPADAT.DW_FECHA_OPERACION_SP BETWEEN '2025-01-01' AND '2025-01-31'

--		AND DW_MUL_SPPADAT.SPCPCO  IN  ( 1, 7  )
	    AND DW_MUL_SPPADAT.SPCPCO  = 7  
...
UNION ALL
	...
	SELECT 
		...
		'Dividelo Todo App' AS Operaci�n,
	...
	WHERE
		...
		DW_BEL_IBLOGDTO.DW_FECHA BETWEEN '2025-01-01' AND '2025-01-31'
...
UNION ALL
	...
	WHERE
		...
		DW_BEL_IBLOGBDT.DW_FECHA BETWEEN '2025-01-01' AND '2025-01-31'
...
UNION ALL
	...
	SELECT
		...
		DW_BEL_IBLGLXUS.IBCCOD AS Operaci�n,
	...
	WHERE   
		...
		DW_BEL_IBLGLXUS.DW_FECHA_CONTROL BETWEEN '2025-01-01' AND '2025-01-31'
...
UNION ALL
	...
	SELECT
		...
		CASE WHEN DW_BEL_IBLGAPPY.IBTYPT = 'C' THEN 'Aprovisionamiento TC' ELSE 'Aprovisionamiento TD' END AS Operaci�n,
	...
	WHERE   
		...
		DW_BEL_IBLGAPPY.DW_FECHA_CONTROL BETWEEN '2025-01-01' AND '2025-01-31'
	...
	SELECT 
		...
		FROM 
			...
			SELECT
				...
				UsuarioActivo=(SELECT SUM(CASE WHEN USSTAT='A' THEN 1 ELSE 0 END) FROM DW_BEL_IBUSER WHERE DW_BEL_IBCLIE.CLCCLI=DW_BEL_IBUSER.CLCCLI),--SUM(UsuarioActivo)OVER(PARTITION BY DW_BEL_IBCLIE.CLCCLI),

				UsuarioInactivo=(SELECT SUM(CASE WHEN USSTAT='I' THEN 1 ELSE 0 END) FROM DW_BEL_IBUSER WHERE DW_BEL_IBCLIE.CLCCLI=DW_BEL_IBUSER.CLCCLI),--SUM(UsuarioInactivo)OVER(PARTITION BY DW_BEL_IBCLIE.CLCCLI),

				CantidadUsuario=(SELECT COUNT(USCODE) FROM DW_BEL_IBUSER WHERE DW_BEL_IBCLIE.CLCCLI=DW_BEL_IBUSER.CLCCLI),--SUM(UsuarioInactivo)OVER(PARTITION BY DW_BEL_IBCLIE.CLCCLI),
...
WHERE
Transacciones.Canal in ( 'App nueva','AP','APP')
AND
Tipo_Cliente = 'N'	
--preguntar ac�
AND
