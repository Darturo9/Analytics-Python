	...
	SELECT * FROM(
		SELECT
			  ...
			  ROW_NUMBER() OVER(PARTITION BY CIF
						ORDER BY fecha_crea desc)	AS rn
		FROM
			DWHBI.dbo.Dw_sms_Perfil_Usuario
		WHERE Dw_sms_Perfil_Usuario.DW_DESCRIPCION_STATUS  IN  ( 'ACTIVO'  )
...
LEFT JOIN
	...
	INNER JOIN 
	usuarios_Bi_movil_Salvador Bi_movil
