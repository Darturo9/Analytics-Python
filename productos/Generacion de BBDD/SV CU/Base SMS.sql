...
WITH usuarios_bi_movil_salvador AS (
    SELECT
        RIGHT('00000000' + RTRIM(LTRIM(CIF)), 8) AS padded_codigo_cliente,
        RTRIM(LTRIM(CIF)) AS codigo_cliente,
        DW_NOMBRE_OPERADOR AS nombre_operador,
        Fecha_Crea AS fecha_crea,
        ROW_NUMBER() OVER (
            PARTITION BY CIF
            ORDER BY Fecha_Crea DESC
        ) AS rn
    FROM DWHBI.dbo.Dw_sms_Perfil_Usuario
    WHERE
        Dw_sms_Perfil_Usuario.DW_DESCRIPCION_STATUS IN ('ACTIVO')
        AND DW_NOMBRE_OPERADOR IN ('CLARO SV', 'Digicel SV', 'MOVISTAR SV', 'TIGO SV')
),
seg_clientes AS (
    ...
    FROM his_cif_clientes clientes
    ...
    INNER JOIN usuarios_bi_movil_salvador bi_movil
        ON RIGHT('00000000' + RTRIM(LTRIM(clientes.cldoc)), 8) = RTRIM(LTRIM(bi_movil.padded_codigo_cliente))
        AND bi_movil.rn = 1
    WHERE
...
contacto AS (
    ...
    SELECT
        ...
        ROW_NUMBER() OVER (
            PARTITION BY seg_clientes.codigo_cliente
            ORDER BY clientes.clnomb, correos.cldire
        ) AS rn
    FROM seg_clientes
    INNER JOIN dw_cif_clientes clientes
        ON seg_clientes.codigo_cliente = RTRIM(LTRIM(clientes.cldoc))
    INNER JOIN dw_cif_direcciones correos
