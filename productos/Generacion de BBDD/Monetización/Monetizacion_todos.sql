/*
==============================================================================
Basado en: Monetizacion.sql (Autor: Monica Isabel García de León - 67382)
Modificación: Eliminar filtro de saldo cero — incluye clientes con cualquier saldo.
...
WITH clientes_cuenta_digital as (
...
), usuarios_cliente_cd as (
	SELECT clientes.*
	FROM usuario_cliente_activo clientes
	INNER JOIN clientes_cuenta_digital cuenta
	ON RTRIM(LTRIM(clientes.cod_cliente)) = RTRIM(LTRIM(cuenta.codigo_cliente))
), usuarios as (
		SELECT DISTINCT cod_cliente FROM usuarios_cliente_cd
)
