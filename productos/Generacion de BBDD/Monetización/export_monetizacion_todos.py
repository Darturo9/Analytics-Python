...
QUERY_PATH = BASE_DIR / "Monetizacion_todos.sql"
EXPECTED_COLUMNS = [
    "codigo_cliente",
    "nombre_cliente",
    "numero_telefono",
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Ejecuta Monetizacion_todos.sql y exporta el resultado a Excel."
    )
    parser.add_argument(
        "--output",
        default="",
        help="Ruta de salida del Excel (opcional). Default: exports/BBDD_MONETIZACION_TODOS_<timestamp>.xlsx",
    )
    ...
    if args.output.strip():
        ...
        output_path = BASE_DIR / "exports" / f"BBDD_MONETIZACION_TODOS_{timestamp}.xlsx"

    exportar_excel(df, str(output_path), hoja="BBDD_MONETIZACION_TODOS")
